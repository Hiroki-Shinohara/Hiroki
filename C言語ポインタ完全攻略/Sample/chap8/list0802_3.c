...
int main(void)
{
	...
	typedef int (*RankFP)(int);		// 関数へのポインタに別名を付ける
	RankFP rfp[] = { isA, isB, isC, isD };

	// ランク別に表示&集計
	printf("=== 評価別一覧 ===\n");
	count = put_list(a, n, rfp[A], "優(A)");
	printf("優(A)人数：%d\n\n", count);

	count = put_list(a, n, rfp[B], "良(B)");
	printf("良(B)人数：%d\n\n", count);

	count = put_list(a, n, rfp[C], "可(C)");
	printf("可(C)人数：%d\n\n", count);

	count = put_list(a, n, rfp[D], "不可(D)");
	printf("不可(D)人数：%d\n\n", count);
